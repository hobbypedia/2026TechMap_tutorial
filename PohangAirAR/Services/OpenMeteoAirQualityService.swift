...
/// Open-Meteo Air Quality API에서 포항 환경 데이터를 가져옵니다.
final class OpenMeteoAirQualityService: AirQualityServiceProtocol, @unchecked Sendable {
    ...
    /// 지정한 좌표의 현재 기온, PM2.5와 UV Index를 병렬로 요청합니다.
    /// - Parameters:
    ///   - latitude: 조회할 위도입니다.
    ///   - longitude: 조회할 경도입니다.
    /// - Returns: 포항이라는 위치 이름과 측정 시각을 포함한 스냅샷입니다.
    ...
    func fetchCurrentAirQuality(
        ...
        do {
            ...
            let weatherURL = try makeURL(
                ...
                queryItems: coordinateItems + [URLQueryItem(name: "current", value: "temperature_2m")]
            )
